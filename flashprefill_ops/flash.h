...
struct Flash_fwd_params : public Qkv_params {
    ...
    bool has_block_sparse;
    int * __restrict__ block_sparse_cu;       // [num_kv_heads * total_q_tiles + 1] — CSR prefix-sum offsets
    int * __restrict__ block_sparse_idx;      // [block_sparse_cu[-1]] — flat compact index of active n_blocks, ascending order
    int total_q_tiles;                        // total Q tiles across all batches (stride for head dimension in segment id)
    int * __restrict__ cu_q_tiles;            // [batch + 1] — cumulative Q tile count per batch (varlen batch offset for segment id)

    // Zero-order mean correction for unselected blocks, fused into the fwd
    // epilogue (Sol-Attn style). k_mean/v_mean hold per-block pooled means,
    // layout [batch, max_k_blocks, num_kv_heads, head_dim(/_v)], produced by
    // the Triton index builder. mean_k_block_size is the logical block size
    // in tokens (= 64 * n_sub); 0 disables the correction.
    void * __restrict__ k_mean_ptr;
    void * __restrict__ v_mean_ptr;
    index_t k_mean_batch_stride, k_mean_block_stride, k_mean_head_stride;
    index_t v_mean_batch_stride, v_mean_block_stride, v_mean_head_stride;
    int mean_k_block_size;
    int mean_n_sub;

    // Block sparse backward (reverse K-centric index, kBlockM=128/kBlockN=64).
    // Segment id = (h_kv * batch + b) * bwd_max_k_tiles + n_block. bwd idx stores
    // local packed_m128 only; a Q-head CTA decodes its own row mask on the fly via
    // (q_pos * bwd_gqa_ratio + head_in_group) / 128 == packed_m128.
    bool has_block_sparse_bwd;
    int * __restrict__ bwd_block_sparse_cu;   // [h_kv * batch * bwd_max_k_tiles + 1]
    int * __restrict__ bwd_block_sparse_idx;  // local packed_m128 per selected entry
    int bwd_max_k_tiles;                      // padded K-tile stride per (h_kv, batch)
    int bwd_gqa_ratio;                        // h / h_k

    // Learnable sink (GPT-OSS style per-head sink logit)
    // Shape: [num_heads] (bfloat16), one learnable scalar per Q-head.
    // Contributes to softmax denominator only, not to attention output.
    void * __restrict__ sinks_ptr;            // [num_heads] — per-head learnable sink logit (same dtype as Q)
    bool has_sink;
