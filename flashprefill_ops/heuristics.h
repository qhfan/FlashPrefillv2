...
inline int num_splits_heuristic(int total_mblocks, int num_SMs, int num_n_blocks, int num_m_blocks, int size_one_kv_head, bool is_causal_or_local, int max_splits, bool has_block_sparse = false) {
    // For block sparse: last_n Q-tiles have O(K) K-tiles while others have O(1),
    // causing severe workload imbalance. Always try splitting to parallelize the
    // last_n Q-tile processing across SMs.
    if (!has_block_sparse && total_mblocks >= 0.8f * num_SMs) {
