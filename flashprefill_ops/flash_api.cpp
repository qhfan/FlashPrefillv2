...
mha_fwd(at::Tensor q,   // (b, s_q, h, d) or (total_q, h, d) if there is cu_seqlens_q
        ...
        int64_t num_splits,
        std::optional<bool> pack_gqa_,
        std::optional<at::Tensor> block_sparse_cu_,   // [num_kv_heads * total_q_tiles + 1] CSR prefix-sum (organized by KV head)
        std::optional<at::Tensor> block_sparse_idx_,  // [block_sparse_cu[-1]] flat compact index, ascending
        std::optional<int64_t> total_q_tiles_,        // total Q tiles across all batches (stride for KV head dimension in segment id)
        std::optional<at::Tensor> cu_q_tiles_,         // [batch + 1] cumulative Q tile count per batch (varlen batch offset)
        std::optional<at::Tensor> sinks_,              // [num_heads] learnable per-head sink logit (GPT-OSS style)
        int64_t sm_margin
        ) {
    ...
    if (params.has_block_sparse) {
        TORCH_CHECK(block_sparse_idx_.has_value(), "block_sparse_idx must be provided if block_sparse_cu is provided");
        TORCH_CHECK(total_q_tiles_.has_value(), "total_q_tiles must be provided if block_sparse_cu is provided");
        TORCH_CHECK(cu_q_tiles_.has_value(), "cu_q_tiles must be provided if block_sparse_cu is provided (varlen batch offset)");
        ...
        TORCH_CHECK(block_sparse_cu.dtype() == torch::kInt32, "block_sparse_cu must have dtype torch.int32");
        TORCH_CHECK(block_sparse_idx.dtype() == torch::kInt32, "block_sparse_idx must have dtype torch.int32");
        TORCH_CHECK(cu_q_tiles.dtype() == torch::kInt32, "cu_q_tiles must have dtype torch.int32");
        params.block_sparse_cu = block_sparse_cu.data_ptr<int>();
        params.block_sparse_idx = block_sparse_idx.data_ptr<int>();
        params.total_q_tiles = total_q_tiles_.value();
        params.cu_q_tiles = cu_q_tiles.data_ptr<int>();
    }
    ...
    if (params.has_sink) {
        ...
        TORCH_CHECK(sinks.scalar_type() == q_type, "sinks must have the same dtype as query");
        TORCH_CHECK(sinks.dim() == 1 && sinks.size(0) == params.h, "sinks must be shape [num_heads]");
...
std::tuple<at::Tensor, at::Tensor, at::Tensor, at::Tensor, at::Tensor> mha_bwd(
    ...
    bool deterministic,
    int64_t sm_margin,
    std::optional<at::Tensor> bwd_block_sparse_cu_,
    std::optional<at::Tensor> bwd_block_sparse_idx_,
    ...
    if (params.has_block_sparse_bwd) {
        TORCH_CHECK(dprops->major == 9, "Block sparse backward only supports SM90");
        TORCH_CHECK(is_causal, "Block sparse backward only supports causal attention");
        // window_size has already been normalized above (causal => window_size_right = 0).
        TORCH_CHECK(window_size_left < 0 && window_size_right == 0, "Block sparse backward does not support sliding window");
        TORCH_CHECK(softcap == 0.0, "Block sparse backward does not support softcap");
        TORCH_CHECK(!deterministic, "Block sparse backward does not support deterministic mode");
        TORCH_CHECK(sm_margin == 0, "Block sparse backward does not support sm_margin");
        TORCH_CHECK(is_varlen_q && is_varlen_k, "Block sparse backward requires varlen q and k");
        TORCH_CHECK(head_size == head_size_v && (head_size_rounded == 128 || head_size_rounded == 256),
                    "Block sparse backward currently supports hdim128/256 with equal qk/v dim");
        TORCH_CHECK(bwd_block_sparse_idx_.has_value() && bwd_max_k_tiles_.has_value(),
        ...
        TORCH_CHECK(bwd_cu.is_cuda() && bwd_idx.is_cuda(), "Block sparse backward index must be CUDA tensors");
        TORCH_CHECK(bwd_cu.dtype() == torch::kInt32 && bwd_idx.dtype() == torch::kInt32,
                    "Block sparse backward index must be int32");
        params.bwd_block_sparse_cu = bwd_cu.data_ptr<int>();
        params.bwd_block_sparse_idx = bwd_idx.data_ptr<int>();
...
void build_block_sparse_index(
    at::Tensor const& q,
    at::Tensor const& k_pool,
    at::Tensor const& page_table,
    std::optional<at::Tensor> const& cu_seqlens_q,
    at::Tensor const& kv_seqlens,
    at::Tensor const& cu_q_tiles,
    at::Tensor const& block_sparse_cu,
    at::Tensor const& block_sparse_idx,
    int64_t batch_size,
    int64_t num_q_heads,
    int64_t num_k_heads,
    int64_t total_q_tiles,
    int64_t max_seqlen_q,
    int64_t max_seqlen_k,
    double alpha,
    ...
    int64_t q_stride_seq = q.stride(0);
    int64_t q_stride_head = q.stride(1);
    int64_t k_pool_stride_slot = k_pool.stride(0);
...
TORCH_LIBRARY_IMPL(flashprefill, CUDA, m) {
    m.impl("fwd", &mha_fwd);
    m.impl("bwd", &mha_bwd);
    m.impl("fwd_combine", &mha_combine);
    m.impl("get_scheduler_metadata", &mha_fwd_get_scheduler_metadata);
#if 0  // build_block_sparse_index disabled
    m.impl("build_block_sparse_index", &build_block_sparse_index);
#endif  // build_block_sparse_index disabled
}