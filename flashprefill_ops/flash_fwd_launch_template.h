...
template <int Arch, int kHeadDim, int kHeadDimV, int ClusterM, typename Element, typename ElementOut,
          ...
          bool PackGQA, bool Split, bool V_colmajor, bool Has_BlockSparse, bool Has_Sink>
void run_flash_fwd(Flash_fwd_params &params, cudaStream_t stream) {
    ...
    using CollectiveMainloop = std::conditional_t<
        Arch >= 90,
        flash::CollectiveMainloopFwdSm90<kStages, ClusterShape, TileShape_MNK, kHeadDimV, Element, float, cutlass::arch::Sm90, Is_causal, Is_local, Has_softcap, Varlen, PagedKVNonTMA, AppendKV, HasQv, MmaPV_is_RS, IntraWGOverlap, PackGQA, Split, V_colmajor, Has_BlockSparse, Has_Sink>,
        flash::CollectiveMainloopFwdSm80<kNWarps, kStages, Q_in_regs, TileShape_MNK, kHeadDimV, Element, float, cutlass::arch::Sm80, Is_causal, Is_local, Has_softcap, Varlen, PagedKVNonTMA, AppendKV, PackGQA, Split, Has_Sink>
    >;
    ...
    typename CollectiveMainloop::Arguments mainloop_args {
        ...
        params.has_block_sparse ? params.block_sparse_cu : nullptr,
        params.has_block_sparse ? params.block_sparse_idx : nullptr,
        params.has_block_sparse ? params.total_q_tiles : 0,
        params.has_block_sparse ? params.cu_q_tiles : nullptr,
        // Zero-order mean correction for unselected blocks (fwd epilogue)
        static_cast<Element const*>(params.k_mean_ptr),
        static_cast<Element const*>(params.v_mean_ptr),
        params.k_mean_batch_stride, params.k_mean_block_stride, params.k_mean_head_stride,
        params.v_mean_batch_stride, params.v_mean_block_stride, params.v_mean_head_stride,
        params.mean_k_block_size, params.mean_n_sub,
        // Learnable sink (GPT-OSS style per-head sink logit)
        params.has_sink ? static_cast<Element const*>(params.sinks_ptr) : nullptr
    };
                ...
                BOOL_SWITCH(params.qv_ptr, HasQV_, [&] {
                    ...
                    APPENDKV_SWITCH(params.knew_ptr, AppendKV, [&] {
                        ...
                        CLUSTER_SWITCH(cutlass::ceil_div(params.seqlen_q * (!PackGQA ? 1 : params.h / params.h_k), kBlockM) % 2 == 0, Use_cluster, [&] {
                            ...
                            BLOCK_SPARSE_SWITCH(params.has_block_sparse, Has_BlockSparse, [&] {
                                HAS_SINK_SWITCH(params.has_sink, Has_Sink, [&] {
                                    run_flash_fwd<Arch, kHeadDim, kHeadDimV, ClusterM, T, T_out, Is_causal, Is_local, Has_softcap, Varlen, PagedKVNonTMA, AppendKV && Varlen, HasQv, PackGQA, Split, V_colmajor, Has_BlockSparse, Has_Sink>(params, stream);
                                });
                            });
                        });
                    });
