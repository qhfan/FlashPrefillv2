...
template <int Stages, int Stages_dO, int Stages_dS, class ClusterShape_, class TileShape_MNK_, class Element_, class ElementAccum_, class ArchTag_,
        ...
        bool Mma_dP_is_RS=false, bool Has_BlockSparse_=false>
struct CollectiveMainloopBwdSm90 {
    ...
    struct Arguments {
        ...
        int const* const ptr_bwd_block_sparse_cu = nullptr;
        int const* const ptr_bwd_block_sparse_idx = nullptr;
        int const bwd_max_k_tiles = 0;
        int const bwd_gqa_ratio = 1;
    };
    ...
    to_underlying_arguments(Arguments const& args) {
        ...
        return {args.shape_Q, args.shape_K, 
                ...
                args.ptr_bwd_block_sparse_cu, args.ptr_bwd_block_sparse_idx,
                args.bwd_max_k_tiles, args.bwd_gqa_ratio};
    }
    ...
    CUTLASS_DEVICE
    static cute::tuple<int, int> bwd_block_sparse_range(Params const& params, int n_block, int bidh, int bidb) {
        int const bidh_kv = params.qhead_per_khead_divmod.divide(bidh);
        int64_t const sid = (int64_t(bidh_kv) * params.num_batch + bidb) * params.bwd_max_k_tiles + n_block;
        int const lo = params.ptr_bwd_block_sparse_cu[sid];
        int const hi = params.ptr_bwd_block_sparse_cu[sid + 1];
        return {lo, hi - lo};
    }

    CUTLASS_DEVICE
    static int bwd_block_sparse_entry_p(Params const& params, int lo, int i) {
        // Packed group id (kBlockM packed rows), sorted ascending within the segment.
        return params.ptr_bwd_block_sparse_idx[lo + i];
    }

    CUTLASS_DEVICE
    static int bwd_block_sparse_entry_m_block(Params const& params, int lo, int i) {
        return bwd_block_sparse_entry_p(params, lo, i) / params.bwd_gqa_ratio;
    }
    ...
    load(Params const& params,
        ...
        int m_block_min = 0, m_block_max = 0, num_entries = 0, bs_lo = 0;
        if constexpr (Has_BlockSparse) {
            auto [lo, cnt] = bwd_block_sparse_range(params, n_block, bidh, bidb);
            bs_lo = lo;
            num_entries = cnt;
            if (num_entries == 0) {
                scheduler_prefetch();
        ...
        auto entry_m_block = [&](int i) {
            if constexpr (Has_BlockSparse) { return bwd_block_sparse_entry_m_block(params, bs_lo, i); }
            else { return m_block_min + i; }
        };
        ...
        if (lane_predicate) {
            ...
            for (int entry_idx = 0; entry_idx < num_entries - 1; ++entry_idx) {
                int const m_block_next = entry_m_block(entry_idx + 1);
                // Sorted entries of one q tile are contiguous; load Q/dO once per tile.
                if constexpr (Has_BlockSparse) {
                    if (m_block_next == m_block) { continue; }
                }
                ...
                copy(bulk_copy.with(*pipeline_q.producer_get_barrier(smem_pipe_write)),
                     gLSE(_, m_block_next), sLSE(_, smem_pipe_write.index()));
    ...
    store_dq(Params const& params,
        ...
        for (int entry_idx = 0; entry_idx < num_entries; ++entry_idx) {
            m_block = entry_m_block(entry_idx);
            // Sorted entries of one q tile are contiguous; store dQ once per tile.
            if constexpr (Has_BlockSparse) {
                if (entry_idx > 0 && m_block == entry_m_block(entry_idx - 1)) { continue; }
            }
    ...
    mma(Params const& params,
        ...
        int m_block_min = 0, m_block_max = 0, num_entries = 0, bs_lo = 0;
        if constexpr (Has_BlockSparse) {
            int const bidh_tmp = get<1>(block_coord);
            auto [lo, cnt] = bwd_block_sparse_range(params, n_block, bidh_tmp, bidb);
            bs_lo = lo;
            num_entries = cnt;
        ...
        static constexpr int BSRow = !SdP_swapAB ? 0 : 1;
        Tensor cBS = cute::make_identity_tensor(Shape<Int<!SdP_swapAB ? kBlockM : kBlockN>, Int<!SdP_swapAB ? kBlockN : kBlockM>>{});
        Tensor tScBS = thread_mma_SdP.partition_C(cBS);
        Tensor tScBS_rowcol = make_tensor(tScBS.data(), flash::convert_layout_acc_rowcol</*Transposed=*/SdP_swapAB>(tScBS.layout()));
        int const bs_head_in_group = [&] {
            if constexpr (Has_BlockSparse) {
                int const bidh_kv = params.qhead_per_khead_divmod.divide(bidh);
                return bidh - bidh_kv * params.bwd_gqa_ratio;
            } else {
                return 0;
            }
        }();
        ...
        auto bwd_step = [&](int m_block, auto mask_fn, uint32_t bs_bitmap = 0) {
            ...
            for (int mi = 0; mi < size<0>(scores); ++mi) {
                ...
                if constexpr (Has_BlockSparse) {
                    int const q_row = get<BSRow>(tScBS_rowcol(mi, _0{}));
                    int const q_pos = q_row + m_block * kBlockM;
                    // The row is selected iff its packed group e is present in the merged
                    // bitmap of this q tile (bit e % gqa_ratio).
                    int const e = (q_pos * params.bwd_gqa_ratio + bs_head_in_group) / kBlockM;
                    bool const row_valid = q_pos < seqlen_q &&
                        (((bs_bitmap >> (e - m_block * params.bwd_gqa_ratio)) & 1u) != 0u);
                    #pragma unroll
                    for (int ni = 0; ni < size<1>(scores); ++ni) {
                        scores(mi, ni) = !row_valid ? 0.f
        ...
        if constexpr (Has_BlockSparse) {
            auto mask_fn = [&](auto& tSrS, int m_block) { mask.template apply<true /*Seqlenk_mask*/, Is_causal && !SeparateMaskingIterations, false /*Local_mask*/>(tSrS, m_block, n_block); };
            // Entries are sorted, so the up-to-gqa_ratio entries of one q tile are
            // contiguous. Merge them into a single bwd_step with a row bitmap.
            int prev_m = -1;
            CUTLASS_PRAGMA_NO_UNROLL
            for (int entry_idx = 0; entry_idx < num_entries; ++entry_idx) {
                m_block = bwd_block_sparse_entry_m_block(params, bs_lo, entry_idx);
                if (m_block == prev_m) { continue; }
                prev_m = m_block;
                uint32_t bs_bitmap = 0;
                for (int j = entry_idx; j < num_entries; ++j) {
                    int const e2 = bwd_block_sparse_entry_p(params, bs_lo, j);
                    if (e2 / params.bwd_gqa_ratio != m_block) { break; }
                    bs_bitmap |= 1u << (e2 - m_block * params.bwd_gqa_ratio);
                }
                bwd_step(m_block, mask_fn, bs_bitmap);
            }
        } else {
            // We have separate iterations with causal masking. Not necessary for hdim 128 but for hdim 64
            // this helps quite a bit to not have to do causal masking for most of the iterations.
