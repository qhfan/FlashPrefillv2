...
def create_flashprefill_backend(runner):
    from sglang.srt.layers.attention.flashprefill_backend import (
        FlashPrefillAttnBackend,
    )

    # Block-sparse hyper-params come from CLI server args
    # (--flashprefill-*). Their defaults are defined on ServerArgs and mirror
    # FlashPrefillAttnBackend.__init__, so an unset flag == the backend default.
