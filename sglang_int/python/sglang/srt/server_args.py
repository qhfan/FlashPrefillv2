...
class ServerArgs:
    ...
    flashprefill_window: int = 4
    flashprefill_abs_threshold: float = 1.0
    flashprefill_full_attention_layers: int = 0
    flashprefill_last_n_blocks: int = 2
    flashprefill_min_sparse_q_len: int = 4096
    flashprefill_k_block_n: int = 64
    flashprefill_use_mean_correction: bool = False
    ...
    def add_cli_args(parser: argparse.ArgumentParser):
        ...
        parser.add_argument(
            "--flashprefill-attention-sink",
            type=int,
            default=ServerArgs.flashprefill_attention_sink,
            help="FlashPrefill block-sparse: number of always-selected sink blocks at sequence start.",
        )
        parser.add_argument(
            "--flashprefill-window",
            type=int,
            default=ServerArgs.flashprefill_window,
            help="FlashPrefill block-sparse: local sliding-window size (in blocks) always selected.",
        )
        parser.add_argument(
            "--flashprefill-abs-threshold",
            type=float,
            default=ServerArgs.flashprefill_abs_threshold,
            help="FlashPrefill block-sparse: max-based dynamic threshold (the paper's alpha in (0, 1]); keeps blocks whose tile-level score energy (softmax energy summed over the packed rows of the q tile) >= alpha * max block energy within the same (kv_head, q_tile) segment. Larger alpha means sparser selection. Paper-calibrated: Llama-3.1-8B 0.18, Qwen2.5-7B 0.08 (block size 128).",
        )
        parser.add_argument(
            "--flashprefill-full-attention-layers",
            type=int,
            default=ServerArgs.flashprefill_full_attention_layers,
            help="FlashPrefill block-sparse: keep the first N layers (0-based layer_id < N) on full dense attention via the FA3 fallback; block sparsity only applies from layer N onwards.",
        )
        parser.add_argument(
            "--flashprefill-last-n-blocks",
            type=int,
            default=ServerArgs.flashprefill_last_n_blocks,
            help="FlashPrefill block-sparse: number of trailing blocks always selected (must be >= 1).",
        )
        parser.add_argument(
            "--flashprefill-min-sparse-q-len",
            type=int,
            default=ServerArgs.flashprefill_min_sparse_q_len,
            help="FlashPrefill block-sparse: minimum Q length (new tokens per request) required for every request in an extend batch. Sparse attention is used only when all Q lengths are strictly greater than this value; otherwise the whole batch uses FA3. Set to 0 to disable short-query fallback for positive Q lengths.",
        )
        parser.add_argument(
            "--flashprefill-k-block-n",
            type=int,
            default=ServerArgs.flashprefill_k_block_n,
            help="FlashPrefill block-sparse: logical K-block size in tokens for scoring/selection. Must be a power-of-two multiple of 64 (64/128/256/512). Larger logical blocks are expanded into consecutive 64-token attention tiles by the index builder, so no kernel rebuild is needed. Note: attention-sink/window are counted in these logical blocks.",
        )
        parser.add_argument(
            "--flashprefill-use-mean-correction",
            action="store_true",
            help="FlashPrefill block-sparse: enable the zero-order mean correction. Unselected KV blocks contribute their pooled K/V means inside the attention kernel epilogue (forward only); the index builder additionally pools V block means.",
        )
        parser.add_argument(
            "--sampling-backend",
            type=str,
